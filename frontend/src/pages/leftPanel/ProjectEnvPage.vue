    ...
    <div>
      ...
      <div class="col-4 border-right bg-dark-soft">
        <div class="q-pa-md row items-center justify-between border-bottom">
          <span class="text-overline text-grey-7 text-weight-bold"
            >Env Variables</span
          >
          <q-btn flat round dense icon="bi-plus-lg" size="xs" color="primary">
            <q-menu
              ...
              anchor="bottom right"
              self="top right"
              class="bg-grey-10 border-lighter"
            >
              <q-list dense style="min-width: 150px">
                <q-item
                  ...
                  v-close-popup
                  v-for="type in ['Database', 'API Key', 'SSH']"
                  :key="type"
                >
                  <q-item-section class="text-caption"
                    >Add {{ type }}</q-item-section
                  >
                </q-item>
              </q-list>
            </q-menu>
          ...
          <q-tab-panel name="dev" class="q-pa-none">
            ...
            >
              ...
              <div class="text-caption text-grey-8 q-mt-sm">
                Select a project to view environment
              </div>
            </div>
            <q-list v-else dense padding class="q-gutter-y-xs">
              <q-expansion-item
                v-for="(items, category) in envData"
                :key="category"
                :label="(category as unknown as string).toUpperCase()"
                header-class="text-overline text-grey-6 text-weight-bold"
                ...
                <q-list dense padding>
                  ...
                  >
                    <q-item-section avatar class="min-width-auto q-pr-sm">
                      <q-icon
                        :name="getIcon(category as unknown as string)"
                        size="14px"
                      />
                    </q-item-section>
                    <q-item-section class="text-caption text-weight-medium">
                      {{ mapLabel(category.toString(), item) }}
                    </q-item-section>
                  </q-item>
                  <div
                    v-if="items.length === 0"
          ...
          <q-tab-panel name="production" class="q-pa-none column">
            <div class="column items-center justify-center col text-grey-8">
              ...
              <div class="text-caption">
                Production settings are encrypted by default
              </div>
              <q-btn
                outline
                color="primary"
...
function mapLabel(category: string, data: any): string {
