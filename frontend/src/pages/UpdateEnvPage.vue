    ...
    <div class=" column no-wrap">
        <div class="q-pa-lg full-height column no-wrap">
            ...
            <div class="row items-center justify-between q-mb-md">
                ...
                <q-btn flat round dense icon="bi-plus-circle" class="text-comment" @click="addNewKey">
            ...
            <div class="row items-center q-mb-xs q-pl-md">
                ...
                <q-input v-model="configName" dark dense borderless placeholder="ConnectionName"
                    input-class="text-mono text-key text-weight-bold" class="editor-input col-4" />
            ...
            <q-scroll-area class="q-pr-md" style="height: calc(100vh - 420px)">
                <div class="q-pl-md">
                    <div v-for="(item, index) in items" :key="index" class="row items-start q-mb-sm no-wrap group">
                        <!-- Indentation Symbol or Space -->
                        <div class="q-mr-sm text-grey-9 text-mono select-none">{{ (index + 1).toString().padStart(2,
                            '0') }}</div>

                        <!-- Key Editor -->
                        <div class="col-4">
                            <q-input v-model="item.key" dark dense borderless placeholder="KEY"
                                input-class="text-mono text-key padding-none" class="editor-input" />
                        </div>
                        ...
                        <!-- Value Editor -->
                        <div class="col">
                            <q-input v-model="item.value" dark dense borderless placeholder='"VALUE"'
                                input-class="text-mono text-value padding-none" class="editor-input" />
                        </div>

                        <!-- Delete Action -->
                        <q-btn flat round dense icon="bi-x" color="red-5" size="md"
            ...
            <div class="row reverse q-mt-lg">
                <q-btn unelevated color="primary" label="Save to Environment" class="border-radius-md" no-caps
                    :loading="saving" @click="saveChanges" />
                <q-btn flat label="Discard" color="grey-7" class="q-mr-sm" no-caps @click="discardChanges" />
            </div>
        </div>
    </div>
</template>
