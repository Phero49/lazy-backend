<template>
  <div class="col column bg-dark-page" v-if="config">
    <div
      class="q-pa-md border-bottom bg-dark-soft row items-center justify-between"
    >
      <div class="row items-center q-gutter-x-sm">
        <q-icon :name="getIcon(category)" color="primary" />
        <span class="text-caption text-weight-bold text-white"
          >{{ category.toUpperCase() }} / {{ name }}</span
        >
        <q-badge
          v-if="store.activeTab?.isReadonly"
          label="READ-ONLY"
          color="grey-9"
          text-color="grey-5"
          class="q-ml-sm"
    ...
    <div class="max-width-600 q-mx-auto">
      <div class="column q-gutter-y-lg">
        ...
        >
          <span class="text-overline text-grey-7 size-9">{{
            (key as unknown as string).toUpperCase()
          }}</span>
          <q-toggle
  ...
  if (store.currentProject && category.value && name.value) {
    ...
    if (categoryData) {
      const item = categoryData.find(
        (i: any) => Object.keys(i)[0] === name.value,
      );
      if (item) {
...
watch(
  () => [route.params.category, route.params.name, store.currentProject],
  loadConfig,
);
