<template>
    <div style="width: 100%;" class="q-pa-md column  full-height">
        <div class="row items-center justify-between q-mb-md">
            ...
            <q-btn flat round dense icon="bi-plus-lg" size="xs" color="primary" @click="addNewProject = true">
        ...
        <template v-else-if="projects.length === 0">
            <div class="column items-center justify-center q-pa-xl text-grey-8">
                ...
                <q-btn flat no-caps label="Create one core" color="primary" class="q-mt-sm"
        ...
        <q-scroll-area v-else style="height: 80vh;width: 100%;">
            <q-list dense dark class="q-gutter-y-xs">
                <q-item clickable v-for="project in projects" :key="project.name" class="project-item"
                    :active="store.currentProject === project.name" active-class="active-project"
                    @click="selectProject(project)">
                    <q-item-section avatar class="min-width-auto q-pr-sm">
                        <q-icon :name="store.currentProject === project.name ? 'bi-folder-fill' : 'bi-folder'"
                            size="14px" :color="store.currentProject === project.name ? 'primary' : 'grey-7'" />
                    </q-item-section>
                    <q-item-section class="text-caption">{{ project.name }}</q-item-section>

                    <q-item-section side v-if="store.currentProject === project.name">
                        <div class="active-dot" />
                    </q-item-section>
                    <q-item-section side v-else>
                        <q-btn color="grey-7" icon="bi-trash" size="xs" flat round dense
        ...
        <q-dialog v-model="addNewProject" persistent>
            <q-card square class="bg-dark-soft border-lighter q-pa-md" style="min-width: 400px">
                <q-card-section class="q-pb-none">
                    <div class="text-h6 text-white text-weight-bold">New Project</div>
                    <div class="text-caption text-grey-6">Initialize a new lazy-backend workspace.</div>
                </q-card-section>

                <q-card-section>

                    <q-input dark dense standout v-model="newProjectName" placeholder="e.g. ecommerce-api"
                        label="Project Name" autofocus @keyup.enter="handleCreateProject"
                ...
                <q-card-actions align="right" class="q-pt-md">
                    ...
                    <q-btn unelevated label="Create Project" color="primary" :loading="creating"
                        @click="handleCreateProject" no-caps />
                </q-card-actions>
            </q-card>
        </q-dialog>
    </div>
...
import { ListProjects, CreateNewProject, DeleteProject } from "../../wailsjs/go/main/App";
